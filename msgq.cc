...
int msgq_new_queue(msgq_queue_t * q, const char * path, size_t size){
  ...
  const char* prefix = std::getenv("OPENPILOT_PREFIX");//u can define your own const char*
  if (prefix) {
...
void msgq_init_publisher(msgq_queue_t * q) {
  ...
  *q->num_readers = 0;
  ...
  for (size_t i = 0; i < NUM_READERS; i++){//wht no atomicity here on
...
int msgq_msg_send(msgq_msg_t * msg, msgq_queue_t *q){
  ...
  if (q->write_uid_local != *q->write_uid){
    std::cout << "Killing old publisher: " << q->endpoint << std::endl;//this means we need to re init the new pub on this
    errno = EADDRINUSE;
    return -1;
  }
